...
func (s *Service) Claim(ctx context.Context, patientID, accountID string, version int, mc MemoryClass) error {
	return s.repo.Claim(ctx, patientID, accountID, version)
}

func memoryClassGuard(mc MemoryClass, from, to State) error {
	// Empty memory class defaults to persistent for backward compatibility.
	if mc == "" {
		mc = MemoryClassPersistent
	}
...
type Repository interface {
	...
	Claim(ctx context.Context, patientID, accountID string, version int) error
...
func (r *SQLRepository) Get(ctx context.Context, patientID string) (*Record, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT patient_id, account_id, state, version, lease_expires_at, updated_at
		FROM ownership_records WHERE patient_id = $1
	`, patientID)
	var rec Record
	var lease sql.NullTime
	if err := row.Scan(&rec.PatientID, &rec.AccountID, &rec.State, &rec.Version, &lease, &rec.UpdatedAt); err != nil {
...
func (r *SQLRepository) Claim(ctx context.Context, patientID, accountID string, version int) error {
	res, err := r.db.ExecContext(ctx, `
		INSERT INTO ownership_records (patient_id, account_id, state, version, lease_expires_at)
		VALUES ($1, $2, 'owned', 1, NOW() + INTERVAL '24 hours')
		ON CONFLICT (patient_id) DO UPDATE SET
			...
			updated_at = NOW()
		WHERE ownership_records.state = 'pool' AND ownership_records.version = $3
	`, patientID, accountID, version)
	if err != nil {
		return err
	}
...
func (r *InMemoryRepository) Claim(ctx context.Context, patientID, accountID string, version int) error {
	rec, ok := r.records[patientID]
	if !ok {
	...
	if rec.State != StatePool || rec.Version != version {
